...
export function FleetShowcase() {
  return (
    <section className="rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.08] to-white/[0.03] p-5">
      ...
      <p className="mt-1 text-sm text-slate-300">Premium multimodal fleet supporting last-mile through special cargo operations.</p>

      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        {fleetItems.map((item) => (
          <article key={item.title} className="rounded-xl border border-white/10 bg-slate-950/50 p-3 transition hover:border-cyan-300/40">
            <div className="overflow-hidden rounded-lg border border-white/10 bg-slate-900/40">
              <img src={item.image} alt={`${item.title} fleet visual`} className="h-36 w-full object-cover" />
            </div>
